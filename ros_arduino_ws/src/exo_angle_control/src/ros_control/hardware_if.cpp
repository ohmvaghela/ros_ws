...
public:
  BLDCRobot()
  {
    ...
    void hlCallback(const exo_angle_control::EncoderHL::ConstPtr& msg)
    {
        hl_angle_ = msg->angle;
    }
    
    void klCallback(const exo_angle_control::EncoderKL::ConstPtr& msg)
    {
        kl_angle_ = msg->angle;
    }
    
    void krCallback(const exo_angle_control::EncoderKR::ConstPtr& msg)
    {
        kr_angle_ = msg->angle;
    }
    
    void hrCallback(const exo_angle_control::EncoderHR::ConstPtr& msg)
    {
        hr_angle_ = msg->angle;
    }
 ...
 ros::Time get_time() const
  ...
  {
    return ros::Duration(1.0 / CONTROL_FREQUENCY);
  }
  void write()
  {
    for (int i = 0; i < 4; ++i)
